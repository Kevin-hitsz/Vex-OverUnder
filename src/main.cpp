...
void skill_new2(){      //机创赛版本2 上方导球22->推入联队球->爬杆  用时约55s
	...
	/* step_2 将接触的联队球与放置在球门的联队球推入球门 */
	closemove(0.14, -0.60, 31.52,1500);
	openmove(0,-1,0,300);
	openmove(0,1.58,0,500);
	openmove(0,0,0,200);
	/* step_2 end */

	/* step_3 爬杆 */
	openmove(0,-1,0,300);
	closemove(0.21, 0.35, 36.37,1000);
	closemove(0.86, 0.82, 35.5,2000);
	Climber();
	pros::delay(500);
	openmove(0.8,0,0,600);
	Climber();
	/* step_3 end */
...
void skill_new3(){		//机创赛版本3 上方翅膀导入
	...
	openmove(0.0, 1,  0, 500);
	openmove(0.0, 0.0,  0.0, 200);
	openmove(0.0, -1,  0, 300);
	...
	// 	openmove(0.0, 0.0,  0.0, 200);
	// 	openmove(0.0, 0.0, 8.0, 270);
	// 	openmove(0.0, 0.0,  0.0, 600);

	// 	if (fabs(RopoDevice::Sensors::Encoder.GetPosX()/1000 - 0.27) >= 0.03 || fabs(RopoDevice::Sensors::Encoder.GetPosY()/1000 - 0.39) >= 0.03){
	// 		closemove(0.27, 0.39, -100, 1000);
	// 	}
	// }
	// ExternRight();
